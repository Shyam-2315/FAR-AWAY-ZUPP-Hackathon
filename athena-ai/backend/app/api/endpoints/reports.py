...
GET /api/reports/{event_id}/pdf
    ...
    Fetches the event and its latest associated Report record from PostgreSQL,
    then formats all agent outputs into a downloadable file attachment using
    ReportLab.  Requires VIEWER role or above.
"""
...
def _build_pdf(event_data: dict) -> bytes:  # type: ignore[type-arg]
    ...
    Layout sections:
        ...
        5. Financial Impact   — estimated_savings + confidence_score
        6. Agent Pipeline     — observation → investigation → prediction →
                                strategies → decision (one section each)
    ...
    # Custom styles that match the dark-themed dashboard aesthetic as
    # closely as a PDF allows.
    ...
    confidence_raw = report.get("confidence_score")
    ...
    # ------------------------------------------------------------------ #
    # 6. Agent pipeline outputs
    # ------------------------------------------------------------------ #
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#e2e8f0")))
    story.append(Paragraph("Agent Pipeline Outputs", style_h2))

    pipeline = event_data.get("pipeline") or {}
    _section("Observer — Observation", pipeline.get("observation"))
    _section("Investigator — Root Cause Analysis", pipeline.get("investigation"))
    _section("Predictor — Financial Exposure", pipeline.get("prediction"))
    _section("Strategy Agent — Options", pipeline.get("strategies"))
    _section("Decision Engine — Chosen Action", pipeline.get("decision"))
...
@router.get(
    ...
    description=(
        "Fetches the event and its latest agent Report record from PostgreSQL, "
        "then streams a formatted PDF file as a downloadable attachment. "
...
async def download_report_pdf(
    ...
    latest_report = None
    if event.reports:
        # Sort descending by created_at and take the first entry.
    ...
    # ------------------------------------------------------------------ #
    # Assemble a flat dict that _build_pdf() can work with cleanly.
    # Keys map directly to the PDF section helpers defined above.
    # ------------------------------------------------------------------ #
    ...
    if latest_report is not None:
        report_dict = {
            ...
            "confidence_score": getattr(latest_report, "confidence_score", None),
        }
        pipeline_dict = {
            "observation": getattr(latest_report, "observation", None),
            "investigation": getattr(latest_report, "investigation", None),
            "prediction": getattr(latest_report, "prediction", None),
            "strategies": getattr(latest_report, "strategies", None),
            "decision": getattr(latest_report, "decision", None),
        }
